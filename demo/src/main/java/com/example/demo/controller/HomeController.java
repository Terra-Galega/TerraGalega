...
public class HomeController {
    ...
    public String account(Model model, HttpSession session) {
        ...
        // http://localhost:8090/account/edit -> formulario para editar MI perfil
    // (el propio Client logueado, no el admin)
    ...
    public String updateAccount(
        ...
        Client actual = (Client) session.getAttribute(SESSION_Client);
        if (actual == null) {
            return "redirect:/login";
        }
        ...
        boolean emailEnUso = clientService.getAllClients().stream()
                .anyMatch(c -> !c.getId().equals(actual.getId()) && c.getEmail().equalsIgnoreCase(email));
        if (emailEnUso) {
        ...
        // Se arma el Client actualizado a partir del que ya estaba en sesión, para
        // no permitir que el propio Client se cambie a sí mismo "admin" ni "active"
        // desde este formulario (eso lo sigue manejando únicamente el admin).
        Client actualizado = new Client(
                actual.getId(),
                name,
                lastName,
                email,
                (password == null || password.isBlank()) ? actual.getPassword() : password,
                phone,
                address,
                actual.getActive(),
                actual.getAdmin());

        Client guardado = clientService.updateClient(actual.getId(), actualizado);
        session.setAttribute(SESSION_Client, guardado);
        return "redirect:/account";
    }
